...
class HTTPRPCTransport extends RPC.Transport
{
    ...
    async HandleIncomingCall(json:RPC.Call,profile?:any):Promise<RPC.Response>
    {
        if (g_logAll)
    ...
    async Call<T extends RPC.Call>(c:T,options?:RPC.CallOptions):Promise<RPC.Response>
    {
        if (!g_isBrowser)
        {
...
export var HTTPRPC = new HTTPRPCTransport();